...
void setmenuitem(short menuitem) {
	if(SDL_GetTicks()-game_state.lastmenuchange>50) {
		game_state.menuitem=menuitem;
		game_state.lastmenuchange=SDL_GetTicks();
	}
}

short getmenuitem() {
	return game_state.menuitem;
}
...
void drawmainmenu(void) {
	...
	char menutext[4][255]={"New Game","Load Game","Options","Exit"};
	int choice=0;
	unsigned long c;
	bool finish=false;
	long lasttime=0;
	...
	for(i=0;i<4;i++) {
		...
		if(game_state.menuitem!=i)
			draw_text(world_pixels, menutext[i], X, 30+((i*20)+20), SDL_MapRGB(world_pixels->format, 255, 255,255), Font.Info, Font.Style);
		else {
			draw_text(world_pixels, menutext[i], X, 30+((i*20)+20), SDL_MapRGB(world_pixels->format, 255, 239, 0), Font.Info, Font.Style);
			//printf("%s\n",menutext[i]);
		}
	}
...
/*	SceCtrlData pad;
	...
	while (!finish) {
		if(SDL_GetTicks()-lasttime>50) {
			...
			if (pad.Buttons != 0) 	{
				if (pad.Buttons & PSP_CTRL_CROSS) finish = true;
				if (pad.Buttons & PSP_CTRL_UP) game_state.menuitem--;
				if (pad.Buttons & PSP_CTRL_DOWN) game_state.menuitem++;
				if (game_state.menuitem<0) game_state.menuitem=0;
				if (game_state.menuitem>3) game_state.menuitem=3;
			}
	...
	/*while(!finish) {
		if(SDL_GetTicks()-lasttime>50) {
			...
			if(c&GP2X_UP) game_state.menuitem--;
			if(c&GP2X_DOWN) game_state.menuitem++;
			if(game_state.menuitem<0) game_state.menuitem=0;
			if(game_state.menuitem>3) game_state.menuitem=3;
			
			if(c&GP2X_X) finish=true;
			lasttime=SDL_GetTicks();
		}
	}*/
...

{
	...
	printf("Displaying PSP-specific menu..\n");
...
/*	struct color_table *system_colors = build_8bit_system_color_table();
	...
	printf("Processing selected menu item: %d\n",game_state.menuitem);
	joystick = SDL_JoystickOpen(0);
	do_menu_item_command(mInterfacePSP,game_state.menuitem,false); 
#else
	game_state.main_menu_display_count++;
#endif
}
...
//#endif
				...
				case iQuitGame:
					{
						bool really_wants_to_quit= true;
#ifdef PSP
						printf("Quitting AlephOne...\n");					
#endif
						switch(game_state.user)
						{
							case _single_player:
#ifdef PSP
								printf("Quitting singe player game...\n");					
#endif
...
/* If you want to quit on command-q while in the game.. */
		...
		printf("PSP - Ingame Menu\n");
		
		case mGamePSP:
			switch(menu_item)
			{
		/*		case iRevertPSP:
					printf("PSP - Revert Game\n");
					handle_load_game();
					break;	*/				
				
				case iCloseGamePSP:
					printf("PSP - Close Game\n");
		...
		case mInterface:
			switch(menu_item)
			{
				...
				case iLoadGame:
				#ifdef PSP
					printf("PSP - Load Game from mInterface\n");
				#endif
					handle_load_game();
					break;
			...
			break;
#ifdef PSP
		/*Sony PSP Specific code for menus */	
			case mInterfacePSP:
				printf("PSP Menu item selected\n");
				switch(menu_item)
				{
					case iNewGamePSP:
						printf("PSP - New Game\n");
					...
					case iLoadGamePSP:
						printf("PSP - Load Game\n");
						handle_load_game();
						break;
						
					case iPreferencesPSP:
						printf("PSP - Preferences\n");
						do_preferences();
						game_state.phase= TICKS_UNTIL_DEMO_STARTS;
						game_state.last_ticks_on_idle= machine_tick_count();
						ForceRepaintMenuDisplay();
...
#ifdef PSP
	printf("Displaying Quit Screens\n");
#endif
	...
	bool cheat)
{
	...
	printf("begin_game started with user: %d\n",user);
		...
		case _single_player:
			...
			starts[0].identifier = 0;
                        ...
                        memset(entry.level_name,0,66);
												printf("Construct single player start...");
                        construct_single_player_start(starts, &number_of_players);
                        printf("Done\n");

			printf("Filling game informations...");
	...
	if(success)
	{
		...
		{
			printf("Displaying chapter screen...");
			FindLevelMovie(entry.level_number);
			show_movie(entry.level_number);
...
{
#ifdef PSP
	printf("PSP - Called Handle Load Game\n");
#endif
