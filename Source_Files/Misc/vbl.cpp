...
uint32 parse_keymap(void)
{
  ...
  if(get_keyboard_controller_status())
	...
	//	MK: I MUST absolutly clean this (TODO-TODO-TODO)
	//		the best way to do this is actually using
	//		aleph one integrated key mapping capabilities

	static SceCtrlData psp_pad;
	
	sceCtrlReadBufferPositive(&psp_pad, 1);
	
	if (PSP_PRESSED(psp_pad, PSP_CTRL_LEFT) && !PSP_PRESSED(psp_pad, PSP_CTRL_TRIANGLE))
		flags |= _turning_left;
		
	if (PSP_PRESSED(psp_pad, PSP_CTRL_RIGHT) && !PSP_PRESSED(psp_pad, PSP_CTRL_TRIANGLE))
		flags |= _turning_right;
		
	if (PSP_PRESSED(psp_pad, PSP_CTRL_UP) && !PSP_PRESSED(psp_pad, PSP_CTRL_TRIANGLE))
		flags |= _moving_forward;
		
	if (PSP_PRESSED(psp_pad, PSP_CTRL_DOWN) && !PSP_PRESSED(psp_pad, PSP_CTRL_TRIANGLE))
		flags |= _moving_backward;
		
	if (PSP_PRESSED(psp_pad, PSP_CTRL_CROSS))
		flags |= _left_trigger_state;
		
	if (PSP_PRESSED(psp_pad, PSP_CTRL_SQUARE))
		flags |= _right_trigger_state;
		
	if (PSP_PRESSED(psp_pad, PSP_CTRL_CIRCLE))
		flags |= _action_trigger_state;
		
	if (PSP_PRESSED(psp_pad, PSP_CTRL_LTRIGGER))
		flags |= _sidestepping_left;
		
	if (PSP_PRESSED(psp_pad, PSP_CTRL_RTRIGGER))
		flags |= _sidestepping_right;
		
	if (PSP_PRESSED(psp_pad, PSP_CTRL_SELECT))
		flags |= _toggle_map;
		
	if (PSP_PRESSED(psp_pad, PSP_CTRL_LTRIGGER) && PSP_PRESSED(psp_pad, PSP_CTRL_RTRIGGER))
		flags |= _swim;
	...
	/*
	 * MK: Look only if not in overhead map as looking share the same
	 *		controls of zooming
	 */
#if 0	 
