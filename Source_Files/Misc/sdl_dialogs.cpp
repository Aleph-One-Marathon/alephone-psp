...
void dialog::event(SDL_Event &e)
{
  ...
  if (active_widget)
	...
	//if(c&GP2X_START) quit(-1);
	//if(c&GP2X_UP||c&GP2X_LEFT) activate_prev_widget();
	//if(c&GP2X_DOWN||c&GP2X_RIGHT) activate_next_widget();
	//if(c&GP2X_X||c&GP2X_L) active_widget->click(0,0);
		...
		case SDL_KEYDOWN:
			...
			case SDL_JOYBUTTONUP:
				switch (e.jbutton.button) {
					case 1:		// CIRCLE = Exit dialog
						quit(-1);
						break;
					case 8:			// Up = Activate previous widget
					case 7:    // LEFT
						activate_prev_widget();
						break;
					case 6:			// Down = Activate next widget
					case 9:
						activate_next_widget();
						break;
				case 2: 		// CROSS = Action on widget
				    active_widget->click(0, 0);
				    break;
				case 10:			// SELECT = Screen dump
						dump_screen();
						break;
					default:
						break;
...
bool dialog::process_events()
{
	while (!done) {
		...
		e.type = SDL_NOEVENT;
		SDL_PollEvent(&e);
		//if(c&GP2X_LEFT||c&GP2X_UP||c&GP2X_RIGHT||c&GP2X_DOWN||c&GP2X_X||c&GP2X_L||c&GP2X_Y) {
