...
// GP2X specific stuff
gp2xkey gp2xkeybindings[14];
gp2xkey gkbdefault1[14];
gp2xkey gkbdefault2[14];
gp2xkey gkbdefault3[14];
 ...
 */
 #ifdef psp
	 int w_debug(char str[],int ret=1)
	{
			/*int len;
			char buffer[50];
			int file;
	...
	void)
{
	...
	//gp2xbindings gp2xkeybindings[16];
	/*gkbdefault1[VK_UP].set1 = _looking_up;
	gkbdefault1[VK_DOWN].set1 = _looking_down;
	gkbdefault1[VK_LEFT].set1 = _turning_left;
	gkbdefault1[VK_RIGHT].set1 = _turning_right;
	gkbdefault1[VK_A].set1 = _sidestepping_left;
	gkbdefault1[VK_B].set1 = _sidestepping_right;
	gkbdefault1[VK_X].set1 = _moving_backward;
	gkbdefault1[VK_Y].set1 = _moving_forward;
	gkbdefault1[VK_VOLUP].set1 = _cycle_weapons_forward;
	gkbdefault1[VK_VOLDOWN].set1 = _action_trigger_state;
	gkbdefault1[VK_L].set1 = _right_trigger_state;
	gkbdefault1[VK_R].set1 = _left_trigger_state;
	gkbdefault1[VK_CLICK].set1 = _looking_center;
	gkbdefault1[VK_SELECT].set1 = _toggle_map;

	gkbdefault2[VK_Y].set1 = _looking_up;
	gkbdefault2[VK_X].set1 = _looking_down;
	gkbdefault2[VK_A].set1 = _turning_left;
	gkbdefault2[VK_B].set1 = _turning_right;
	gkbdefault2[VK_LEFT].set1 = _sidestepping_left;
	gkbdefault2[VK_RIGHT].set1 = _sidestepping_right;
	gkbdefault2[VK_DOWN].set1 = _moving_backward;
	gkbdefault2[VK_UP].set1 = _moving_forward;
	gkbdefault2[VK_VOLUP].set1 = _cycle_weapons_forward;
	gkbdefault2[VK_VOLDOWN].set1 = _action_trigger_state;
	gkbdefault2[VK_L].set1 = _right_trigger_state;
	gkbdefault2[VK_R].set1 = _left_trigger_state;
	gkbdefault2[VK_CLICK].set1 = _looking_center;
	gkbdefault2[VK_SELECT].set1 = _toggle_map;

	gkbdefault3[VK_Y].set1 = _cycle_weapons_forward;
	gkbdefault3[VK_X].set1 = _left_trigger_state;
	gkbdefault3[VK_A].set1 = _cycle_weapons_backward;
	gkbdefault3[VK_B].set1 = _right_trigger_state;
	gkbdefault3[VK_LEFT].set1 = _turning_left;
	gkbdefault3[VK_RIGHT].set1 = _turning_right;
	gkbdefault3[VK_DOWN].set1 = _moving_backward;
	gkbdefault3[VK_UP].set1 = _moving_forward;
	gkbdefault3[VK_LEFT].set2 = _turning_left;
	gkbdefault3[VK_RIGHT].set2 = _turning_right;
	gkbdefault3[VK_DOWN].set2 = _looking_down;
	gkbdefault3[VK_UP].set2 = _looking_up;
	gkbdefault3[VK_VOLUP].set1 = _toggle_map;
	gkbdefault3[VK_VOLDOWN].set1 = _action_trigger_state;
	gkbdefault3[VK_L].set1 = _sidestepping_left;
	gkbdefault3[VK_R].set1 = _sidestepping_right;
	gkbdefault3[VK_CLICK].set1 = _looking_center;
	gkbdefault3[VK_SELECT].set1 = _toggle_look;*/
	...
	int i;
	for(i=0;i<14;i++) {
		gkbdefault1[i].set2=0;
		gkbdefault2[i].set2=0;
	}
	...
	if (!PrefsInited)
	{
		...
		if ((f = fopen("./gp2x.conf","rb")) == NULL) {
		} else {
			if (fread(&input_preferences->controlsetup,sizeof(uint16),1,f) != 1) {
			} else
			if (fread(&input_preferences->invert,sizeof(bool),1,f) != 1) {
			}
			if (fread(gp2xkeybindings,sizeof(gp2xkey),16,f) != 16) {
			}

			fclose(f);
		}

		if(input_preferences->controlsetup==0) { // Default
			for(i=0;i<16;i++) {
				gp2xkeybindings[i].set1=gkbdefault1[i].set1;
				gp2xkeybindings[i].set2=gkbdefault1[i].set2;
			}
		} else if(input_preferences->controlsetup==1) { // Reversed
			for(i=0;i<16;i++) {
				gp2xkeybindings[i].set1=gkbdefault2[i].set1;
				gp2xkeybindings[i].set2=gkbdefault2[i].set2;
			}
		} else if(input_preferences->controlsetup==2) { // Quake2X
			for(i=0;i<16;i++) {
...
void read_preferences ()
{
	...
	#ifdef psp
		w_debug("		Initializing graphic options...");
	#endif
	input_preferences->input_device=_keyboard_or_game_pad;
	graphics_preferences->screen_mode.size=2;
...
#elif defined(SDL)
	#ifdef psp
		w_debug("		Default directory to prefs directory...");
	#endif
...
#endif
	fprintf(F,"  controlsetup=\"%hu\"\n",input_preferences->controlsetup); // JB
	fprintf(F,"  invert=\"%s\"\n",BoolString(input_preferences->invert)); // JB
	fprintf(F,"</input>\n\n");
#ifdef psp
	w_debug("OK\n");
#endif
...
{
	#ifdef psp
		w_debug("			default_graphics_preferences...");
	#endif
...
{
		#ifdef psp
		w_debug("			default_serial_number_preferences...");
	#endif
...
{
	#ifdef psp
		w_debug("			default_network_preferences...");
	#endif
...
{
	#ifdef psp
		w_debug("			default_player_preferences...");
	#endif
...
{
	#ifdef psp
		w_debug("			default_input_preferences...");
	#endif
	...
	// JB
	preferences->controlsetup = 0;
	preferences->invert = 1;
...
{
	#ifdef psp
		w_debug("			default_environment_preferences...\n");
	#endif
...
#else
	#ifdef psp
	  w_debug("				Default specs...");
		w_debug("				Getting default specs...");
	#endif
	get_default_map_spec(DefaultMapFile);
	#ifdef psp
	w_debug("					Got map default specs...");
	#endif
	get_default_physics_spec(DefaultPhysicsFile);
	#ifdef psp
	w_debug("					Got physic default specs...");
	#endif
	get_default_shapes_spec(DefaultShapesFile);
	#ifdef psp
	w_debug("					Got shapes default specs...");
	#endif
	get_default_sounds_spec(DefaultSoundsFile);
	#ifdef psp
	w_debug("					Got sounds default specs...");
		w_debug("OK\n");
	#endif
#endif

#ifdef psp
		w_debug("				Wadfile Checksum...");
#endif              
...
#ifdef psp
		w_debug("				Fill default files info...");
#endif 
...
bool XML_InputPrefsParser::HandleAttribute(const char *Tag, const char *Value)
{
	...
	else if(StringsEqual(Tag, "controlsetup")) {
		return ReadUInt16Value(Value,input_preferences->controlsetup);
	} 
	else if(StringsEqual(Tag, "invert")) {
		return ReadBooleanValue(Value, input_preferences->invert);
	}
	return true;
}
