...
void w_select::event(SDL_Event &e)
{
	...
	if (e.type == SDL_JOYBUTTONUP) {
		if (e.jbutton.button == 7) { //LEFT
			...
			selection_changed();
			e.type = SDL_NOEVENT;	// Swallow event
		} else if (e.key.keysym.sym == 9) {  //RIGHT
			if (selection >= num_labels - 1)
				selection = 0;
			else
				selection++;
...
//	unsigned long c=gp2x_joystick_read();
	...
	/*if(c&GP2X_L || c&GP2X_R || c&GP2X_LEFT || c&GP2X_RIGHT || c&GP2X_UP || c&GP2X_DOWN || c&GP2X_Y || c&GP2X_X || c&GP2X_A || c&GP2X_B || c&GP2X_SELECT)
	{
		dirty = true;
		binding = false;
...
//	unsigned long c = gp2x_joystick_read();
	...
	if (e.type == SDL_JOYBUTTONUP) {
		...
		if (e.jbutton.button == 7) { //LEFT
			...
			item_selected();
			e.type = SDL_NOEVENT;	// Swallow event
		} else if (e.jbutton.button == 9) { //RIGHT
			set_selection(selection + 1);
			item_selected();
			e.type = SDL_NOEVENT;	// Swallow event
		}
