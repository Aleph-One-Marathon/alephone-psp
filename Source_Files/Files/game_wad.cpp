	...
	struct entry_point *entry_point)
{
	...
	/* If we want to save it, this is an untitled map.. */
				printf("Setting saved game name to default...");
	...
	printf("Initializing map for a new game...");
	 ...
	 * on this information being setup properly, so we do it here instead. */
	 printf("Copy game data into dynamic world...\n");
	 if (dynamic_world != NULL) printf("dynamic_world exists\n");
	 if (game_information != NULL) printf("game_information exists\n");
	 obj_copy(dynamic_world->game_information, *game_information);
	 printf("Done.\n");
	 //printf("Senza sensooooo!!!!\n");
	...
	bool new_game)
{
	...
	if (success)
	{
		...
		printf("Getting game Errors with level scripts...");
		short SavedType, SavedError = SavedError = get_game_error(&SavedType);
		printf("Done.\n");
		printf("Running level Scripts...");
		...
		if(success) /* successfully switched. (guaranteed except in net games) */
		{
			...
			printf("Placing inital objects...");
			place_initial_objects();
			printf("Done.\n");
	
			printf("Initializing control panels...");
