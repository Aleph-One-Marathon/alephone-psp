...
// MK: PSP elf informations
#ifdef PSP
#define MODULE_NAME "AlephOne"
PSP_MODULE_INFO("AlphOne", 0, 0, 71);
...
#ifdef PSP
PSPSDLMouse* 		psp_mouse = NULL;
PSPSDLProfilerMg	psp_profiler;
#endif

// MK: On the PSP system we need to setup the exit
//		callback to safely return to XMB
...
{ 
   ...
   thid = sceKernelCreateThread("update_thread", callback_thread, 
...
//	MK: Usal way to setup the PSP callbacks
#ifdef PSP
	setup_callbacks();
#endif
...
// MK: Here we can initialize our mouse simulator

#ifdef PSP
	psp_mouse = new PSPSDLMouse();
	if (psp_mouse == NULL) {
		fprintf (stderr, "Not enought memory to create PSPSDLMouse\n");
		exit(1);
	}

	psp_mouse->bind_button(PSP_CTRL_CROSS, SDL_BUTTON_LEFT);
	psp_mouse->bind_button(PSP_CTRL_CIRCLE, SDL_BUTTON_RIGHT);
	psp_mouse->bind_button(PSP_CTRL_TRIANGLE, SDL_BUTTON_MIDDLE);

	// MK: Without this when calling SDL_WarpMouse the game
	//		gets stuck in an endless loop (should be investigated)
...
static void shutdown_application(void)
{
	...
	PSP_STRLOG("Shutting down application");
	...
	// MK: Free our PSP mouse simulator
#ifdef PSP
     if (psp_mouse) 
		delete psp_mouse;
#endif
	...
	PSP_STRLOG("Calling kernel sleep thread");
#ifdef PSP
	sceKernelSleepThread();
#endif
	
	PSP_STRLOG("Calling kernel exit game, cya");
#ifdef PSP
	sceKernelExitGame();
#endif
...
static void main_event_loop(void)
{
	...
	while ((game_state = get_game_state()) != _quit_game) {
		...
		if (poll_event) {
			...
			// MK: The mouse simulator needs to be updated before
			//		the input processing routine starts, in this
			//		way the game's logic will see no difference
			//		between a real mouse and its simulation.
...
static void handle_game_key(const SDL_Event &event)
{
	...
	static SceCtrlData psp_pad;
	
	sceCtrlReadBufferPositive(&psp_pad, 1);
	
	if (PSP_PRESSED(psp_pad, PSP_CTRL_START))
		key = SDLK_ESCAPE;
