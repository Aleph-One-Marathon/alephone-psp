...
#ifdef PSP
// PSP: From images_sdl.h
// needed in update_screen
extern struct color_table *build_8bit_system_color_table(void);
// PSP: From media.h
extern short get_media_submerged_fade_effect(short media_index);
#endif
...
void initialize_screen(struct screen_mode_data *mode, bool ShowFreqDialog)
{
	...
	if (!screen_initialized) {
		...
		printf("Initializing color table & stuff\n");
		...
		printf("Allocate the bitmap_definition structure for our GWorld\n");
		...
		printf("Allocate and initialize our view_data structure\n");
...
{
	printf("screen_sdl->enter_screen()\n");
...
static void change_screen_mode(int width, int height, int depth, bool nogl)
{
	printf("screen_sdl->change_screen_mode_a(%i,%i,%i)\n",width,height,depth);
	uint32 flags;
	#ifdef PSP
	flags = SDL_DOUBLEBUF;
	#else
	flags = (screen_mode.fullscreen ? SDL_FULLSCREEN : 0);
	#endif
	...
	if (main_surface == NULL) {
		...
		printf("Can't open video display (%s)\n", SDL_GetError());
		exit(1);
	}
...
void change_screen(int width, int height, int depth, bool nogl)
{
	//printf("screen_sdl->change_screen_mode_a(%i,%i,%i)\n",width,height,depth);
...
{
	printf("screen_sdl->change_screen_mode_b(%i)\n",mode->size);
	...
	if (redraw) {
		...
		#ifdef PSP
			if (msize < NUMBER_OF_VIEW_SIZES-PSP_VS_COUNT)
				msize = NUMBER_OF_VIEW_SIZES-PSP_VS_COUNT;
			if (msize > NUMBER_OF_VIEW_SIZES-1)
				msize = NUMBER_OF_VIEW_SIZES-1;
		#else
...
void render_screen(short ticks_elapsed)
{
	...
	// PSP: Allow only the the supported screen resolutions
...
#ifdef PSP
	int OverallHeight = VS.OverallHeight - (TEST_FLAG(VS.flags, _view_show_HUD) && PSP_display_hud &&
											!player_in_terminal_mode(current_player_index) ? 120 : 0);
#else
	int OverallHeight = VS.OverallHeight - (TEST_FLAG(VS.flags, _view_show_HUD) ? 120 : 0);
#endif
	...
	// HUD location
#ifdef PSP
	int HUD_Offset = (OverallWidth - 480) / 2;
	SDL_Rect HUD_DestRect = {HUD_Offset + ScreenOffsetWidth, OverallHeight + ScreenOffsetHeight, 480, 120};
#else
	int HUD_Offset = (OverallWidth - 640) / 2;
	SDL_Rect HUD_DestRect = {HUD_Offset + ScreenOffsetWidth, OverallHeight + ScreenOffsetHeight, 640, 240};
#endif	
	...
	if (ChangedSize) {
		clear_screen();
#ifdef PSP
		if (TEST_FLAG(VS.flags, _view_show_HUD) && PSP_display_hud)
#else
...
#ifdef PSP
		// PSP: Restore the old palette if we changed it in terminal
		if (!world_view->terminal_mode_active && PSP_must_restore_palette) {
			printf("PSP: Restoring palette\n");
			
			assert_world_color_table(&PSP_palette_to_restore,&PSP_palette_to_restore);
			
			PSP_must_restore_palette = false;
		}
#endif
	...
	if (screen_mode.acceleration == _opengl_acceleration) {
...
#ifdef PSP
		//Disable the hud when viewing terminal in Sony PSP to get some space
		//I hope to find another way soon...
		if (TEST_FLAG(VS.flags, _view_show_HUD) && !player_in_terminal_mode(current_player_index) 
			&& PSP_display_hud) {
#else
		if (TEST_FLAG(VS.flags, _view_show_HUD)) {
#endif
		...
		//Disable the hud when viewing terminal in Sony PSP to get some space
		//I hope to find another way soon...
		if (HUD_RenderRequest && !player_in_terminal_mode(current_player_index) && PSP_display_hud) {
#else
		if (HUD_RenderRequest) {
#endif
			DrawHUD(HUD_DestRect);
...
static void update_screen(SDL_Rect &source, SDL_Rect &destination, bool hi_rez)
{
//	uint32 dx = (320 << 16) / PSP_SCREEN_W;
//	uint32 dy = (240 << 16) / PSP_SCREEN_H;
	...
	if (hi_rez) {
		...
		/*
		 * PSP: Get space for the HUD if we are not in terminal or overhead map
		 */

		if (!world_view->terminal_mode_active && !world_view->overhead_map_active && PSP_display_hud) 
		{	
			SDL_Rect src;
			destination.y -= 35;
			...
			src.y = 0;
			src.w = PSP_SCREEN_W;
			src.h = PSP_SCREEN_H - 26;
			SDL_BlitSurface(world_pixels, &src, main_surface, &destination);
			
		} 
		else SDL_BlitSurface(world_pixels, NULL, main_surface, &destination);
	  ...
	  if (SDL_MUSTLOCK(main_surface)) {
		...
		switch (world_pixels->format->BytesPerPixel) {
			case 1:				
				...
				break;
			case 2:	
				//	rescale((pixel16 *)world_pixels->pixels, world_pixels->pitch, (pixel16 *)main_surface->pixels, main_surface->pitch, PSP_SCREEN_W,PSP_SCREEN_H,dx,dy);
...
{
//	printf("Animating Screen Clut...\nBit Depth: %d\n",bit_depth);
	if (bit_depth == 8) {
	//	printf("Using SDL_SetPalette...\n");
		SDL_Color colors[256];
		...
		//printf("Using SDL_SetGammaRamp...\n");
		
		//FILE* flog = fopen("GammaRamp.txt","a");
		...
		//	if (flog) fprintf(flog,"R: %d G: %d B: %d\n",red[i],green[i],blue[i]);
		}
		//if (flog) fclose(flog);
		
		//if (!option_nogamma)
			if (SDL_SetGammaRamp(red, green, blue) == -1) {
...
void DrawHUD(SDL_Rect &dest_rect)
{
	if (HUD_Buffer) {
		...
		#ifdef psp		
			SDL_Surface* HUD_Buffer_Resized = rescale_surface(HUD_Buffer,PSP_SCREEN_W,360);
			SDL_Surface* HUDOld = HUD_Buffer_Resized;
			HUD_Buffer_Resized = SDL_DisplayFormat(HUD_Buffer_Resized);
			SDL_FreeSurface(HUDOld);
				
			SDL_Rect src_rect = {0, 240, PSP_SCREEN_W, 120};
						
			//dest_rect.x=(PSP_SCREEN_W - 320)/2;        
			dest_rect.x=0;
			dest_rect.y=PSP_SCREEN_H - 120;
			dest_rect.w=PSP_SCREEN_W;                      
			dest_rect.h=120;
		...
		#ifdef psp
			SDL_BlitSurface(HUD_Buffer_Resized, &src_rect, main_surface, &dest_rect);
			SDL_FreeSurface(HUD_Buffer_Resized);
		#else
