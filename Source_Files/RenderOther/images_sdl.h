...
static void draw_picture(LoadedResource &rsrc)
{
	...
	int new_sw = (s->w * PSP_SCREEN_W_4_3) / 640;
	int new_sh = (s->h * PSP_SCREEN_H_4_3) / 320;
	
	SDL_Rect dst_rect = {(video->w - new_sw) / 2, (video->h - new_sh) / 2, new_sw, new_sh};
	...
	// Blit picture to screen
		...
		s = rescale_surface(s,PSP_SCREEN_W_4_3,PSP_SCREEN_H_4_3);
		
		int		   colorsn = s->format->palette->ncolors;
		SDL_Color* colors = new SDL_Color[colorsn];
		
		int		   oldpaln = video->format->palette->ncolors;
		SDL_Color* oldpal = (SDL_Color*)malloc(oldpaln*sizeof(SDL_Color));
		SDL_Color  tmpcol;

		memcpy(colors,s->format->palette->colors,
			   sizeof(SDL_Color)*colorsn);
	
		for (int i=0; i<s->format->palette->ncolors; i++)
			printf("[%d]: %d,%d,%d ",i,colors[i].r,colors[i].g,colors[i].b);

		printf("====================================================\n");

		tmpcol = colors[0];
		colors[0] = colors[colorsn-1];
		colors[colorsn-1] = tmpcol; 

		for (int i=0; i<s->format->palette->ncolors; i++)
			printf("[%d]: %d,%d,%d ",i,colors[i].r,colors[i].g,colors[i].b);
		
		memcpy(oldpal,video->format->palette->colors,
			   sizeof(SDL_Color)*video->format->palette->ncolors);
		
		printf("PSP SETTING PALETTE FOR VIDEO\n");
	
		SDL_SetColors(video,colors,0,colorsn);
		
	#endif
	if (SDL_BlitSurface(s, &src_rect, video, &dst_rect) != 0)
		printf("ERROR: Can't blit surface!!!\n");
	SDL_FreeSurface(s);

	// Update display and free picture surface
	#ifdef PSP
		printf("PSP: Double Buffer = %s\n",((video->flags & SDL_DOUBLEBUF) == SDL_DOUBLEBUF ? "yes" : "no"));
		printf("PSP: Bit Depth = %d\n",video->format->BitsPerPixel);
		printf("PSP: Flipping!\n");
		//SDL_Flip(video);
		printf("PSP RESTORING PALETTE\n");
		SDL_SetColors(video,oldpal,0,oldpaln);
	#else
