	...
	short completion_flag)
{
	...
	/*
	 * PSP: Load system palette so that if we are reading a terminal 
	 *		underwater/liquid it is displayed properly
	 */

	#ifdef PSP
		memcpy((void*)&PSP_palette_to_restore,(void*)visible_color_table,
				sizeof(struct color_table));
				
		assert_world_color_table(world_color_table, world_color_table);
		
		PSP_must_restore_palette = true;
...
#ifdef PSP
		printf("Terminal lines per page: %d\n",lines_per_page);
#endif
...
//	unsigned long c = gp2x_joystick_read();
	...
	//if(c&GP2X_L) raw_flags |= _terminal_page_up;
	//if(c&GP2X_R) raw_flags |= _terminal_page_down;
	//if(c&GP2X_B) raw_flags |= _terminal_next_state;
	//if(c&GP2X_A) raw_flags |= _any_abort_key_mask;
	
	#ifdef psp
		SDL_Joystick *joystick = SDL_JoystickOpen(0);
		
		if (SDL_JoystickGetButton(joystick,PSP_BUTTON_CROSS) == SDL_PRESSED) {
			raw_flags |= _terminal_next_state;
		}
		
		if (SDL_JoystickGetButton(joystick,PSP_BUTTON_L) == SDL_PRESSED) {
			raw_flags |= _terminal_page_down;
		}
		
		if (SDL_JoystickGetButton(joystick,PSP_BUTTON_R) == SDL_PRESSED) {
			raw_flags |= _terminal_page_up;
		}
		
		if (SDL_JoystickGetButton(joystick,PSP_BUTTON_SQUARE) == SDL_PRESSED) {
			raw_flags |= _any_abort_key_mask;
		}
...
#ifdef PSP
	printf("Terminal current line: %d\nStart: %d\nLength: %d\n",current_line,
...
#ifdef PSP
		printf("Displaying terminal screen\n");
		printf("Screen top: %d bottom: %d right: %d left: %d\n",screen_bounds.top,
	...
	char *text_base)
{
	...
	for(index= 0; index<group_count; ++index)
	{
		switch(groups[index].type)
		{
			...
			case _information_group:
				{
					short width= 480; // еее sync (Must guarantee 100 high res!)
	...
	void)
{
	...
	// PSP: Terminal must be aware of the PSP screen smaller size 

#ifdef PSP
	return PSP_TERMINAL_LINES;
#else
